...
baseline:
	$(MLX_GENERATE) --model $(MODEL) --system-prompt "$$(cat prompts/system.txt)" --prompt "$$(cat prompts/user.txt)" --chat-template-config $(CHAT_TEMPLATE_CONFIG) --max-tokens 320 --temp 0
...
generate:
	$(MLX_GENERATE) --model $(MODEL) --adapter-path $(ADAPTER) --system-prompt "$$(cat prompts/system.txt)" --prompt "$$(cat prompts/user.txt)" --chat-template-config $(CHAT_TEMPLATE_CONFIG) --max-tokens 320 --temp 0
